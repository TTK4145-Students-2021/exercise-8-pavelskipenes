...
struct PriorityQueue(T)
{
    ...
    T front()
    {
        assert(!queue.empty, format("Attempting to fetch the front of an empty priority queue of %s", T.stringof));
        return queue[0].val;
    }

    void popFront()
    {
        assert(!queue.empty, format("Attempting to popFront() from an empty priority queue of %s", T.stringof));
...
void main()
{
    ...
    auto cfgs = [
        ...
        ResourceUserConfig(0, 1, 25, 3), ResourceUserConfig(6, 0, 26, 2), // wtf?
        ResourceUserConfig(7, 0, 26, 2), ResourceUserConfig(1, 1, 26, 2), // wtf?
        ResourceUserConfig(2, 1, 27, 2), ResourceUserConfig(3, 1, 28, 2), // wtf?
        ResourceUserConfig(4, 1, 29, 2), ResourceUserConfig(5, 1, 30, 2), // wtf?
    ];
